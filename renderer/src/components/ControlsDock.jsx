  ...
  return (
    ...
    >
      ...
      <div className="flex items-center" style={{ height: 28, borderBottom: '1px solid var(--border)', padding: '0 4px' }}>
        {['obs', 'live', 'fans', 'ai', 'coach', 'sync', 'multiview', 'beauty'].map((tab) => (
          <button
            key={tab}
          ...
          >
            {tab === 'obs' ? '🎬 OBS' : tab === 'live' ? '📊 Live' : tab === 'fans' ? '👥 Fans' : tab === 'ai' ? '🤖 AI' : tab === 'coach' ? '💬 Coach' : tab === 'sync' ? '🔗 Sync' : tab === 'multiview' ? '🎥 Multi-View' : '✨ Filters'}
          </button>
        ))}
